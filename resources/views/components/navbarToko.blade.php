...
<div id="sidebar"
    ...
    <nav>
        ...
        <a class=" text-white py-2.5 px-4 my-4 rounded transition duration-200 flex items-center space-x-2"
            href="{{ url('/dashboardtoko') }}">
            <img src="img/admhome.svg" alt="DashboardAdmin" class="h-6 mr-2">
            <span class="relative group pb-1">Dashboard
                <span
                    class="absolute left-0 bottom-0 w-full h-0.4 bg-white transform scale-x-0 transition-transform duration-200 {{ request()->is('dashboardtoko') ? 'scale-x-100' : 'group-hover:scale-x-100' }}"></span>
            </span>
        </a>
        <a class=" text-white py-2.5 px-4 my-4 rounded transition duration-200 flex items-center space-x-2"
            href="{{ url('/tambahproduk') }}">
            <img src="img/admproduk.svg" alt="TambahProdukToko" class="h-6 mr-2">
            <span class="relative group pb-1">Tambah Produk
                <span
                    class="absolute left-0 bottom-0 w-full h-0.4 bg-white transform scale-x-0 transition-transform duration-200 {{ request()->is('tambahproduk') ? 'scale-x-100' : 'group-hover:scale-x-100' }}"></span>
            </span>
        </a>
        <a class=" text-white py-2.5 px-4 my-4 rounded transition duration-200 flex items-center space-x-2"
            href="{{ url('/profiltoko') }}">
