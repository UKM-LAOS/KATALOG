...
<div class="bg-white shadow rounded-lg p-4 w-60 flex flex-col justify-between">
    <div>
        ...
        <h3 class="text-lg font-semibold h-10">
            {{ $produk->namaproduk }}
        </h3>
        <p class="text-gray-600 mb-5 mt-10">RP.{{ number_format($produk->hargaproduk, 0, ',', '.') }}</p>
    </div>
    <div>
        <a href="/detail-product-guest/{{ $produk->id }}"
            class="block text-center bg-slate-500 text-white py-2 rounded-lg">
            Detail
        </a>
    </div>
</div>
